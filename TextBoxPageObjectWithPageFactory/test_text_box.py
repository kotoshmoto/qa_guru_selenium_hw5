...
# PageFactory: 10 passed, 15 failed
# PageObject:   21 passed, 4 failed
#
# PageFactory имеет на 11 падений больше.
#
# Основная причина различия — ошибки реализации PageFactory:
# 1. open() не возвращает self -> AttributeError / NoneType.
# 2. PageFactory ожидает visibility output_box, хотя в негативных сценариях элемент не должен отображаться.
# Следовательно, дополнительное количество падений связано не с поведением тестируемого приложения, а с реализацией Page Object
...
def test_long_input_fields(driver, field_type, long_string):
    ...
    page.fill_form(**{field_type: long_string})
    page.submit()
